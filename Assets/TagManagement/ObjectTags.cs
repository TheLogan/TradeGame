...
{//use namespace to stop any name conflicts
    ...
    public class ObjectTags : MonoBehaviour
    {
        ...
        {//get the tag component with the required name
            ...
            ObjectTags[] namedTags = allObjectTags.Where(t => t.connected != null && t.connected.groupName == connectedName).ToArray();

            if (namedTags.Length == 0)
            {//if not found
                //look to see if there are any null ones which could be used instead
                namedTags = allObjectTags.Where(t => t.connected == null).ToArray();
                if (namedTags.Length == 0)//if not found
                    return null;//return null
            }//end search for null

            return namedTags[0];//return what should be the only ObjectTag or a null one
        }//end GetTagComponent
